...
@Component({
      ...
      <div class="content-body">
        ...
        <div class="scan-hub" (click)="focusScanner()">
          <div class="status-circle" [class.success]="lastScan()">
             <mat-icon class="pulse-icon">{{ lastScan() ? 'done_all' : 'inventory' }}</mat-icon>
          </div>
          <h2>ITEM LOOKUP</h2>
          <p>Scan a SKU or Location barcode to check its live stock ledger.</p>
          
          <div class="scan-actions">
            <button mat-flat-button color="accent" class="big-camera-btn" (click)="startCameraScan($event)">
                <mat-icon>photo_camera</mat-icon> SCAN BARCODE
            </button>
          ...
          <div class="manual-form slide-in" *ngIf="showManual()">
             <mat-form-field appearance="outline" class="full-width">
                <input matInput [(ngModel)]="manualInput" (keyup.enter)="onManualSubmit()" placeholder="Enter SKU/LPN/LOC...">
        ...
        <div class="history-list" *ngFor="let entry of lookupHistory()">
  ...
  styles: [`
    ...
    .status-circle {
      width: 150px; height: 150px; border-radius: 50%; background: rgba(168, 85, 247, 0.15);
      border: 4px solid #a855f7; display: flex; align-items: center; justify-content: center;
    ...
    .pulse-icon { font-size: 4.5rem; width: 4.5rem; height: 4.5rem; color: #a855f7; }
    ...
    .history-item { 
       background: #1e293b; padding: 1.25rem; border-radius: 20px; display: flex; align-items: center; gap: 1.25rem; margin-bottom: 1rem; 
       border-left: 4px solid #a855f7; border-top: 1px solid rgba(255,255,255,0.05);
    }
    .history-item mat-icon { color: #a855f7; }
...
export class MobileInventoryComponent implements AfterViewInit {
  ...
  onScan(event: any) { this.lookupItem(event.target.value); event.target.value = ''; }
  onManualSubmit() { if(this.manualInput) { this.lookupItem(this.manualInput); this.manualInput = ''; this.showManual.set(false); } }
  lookupItem(id: string) {
    ...
    this.lastScan.set(id); setTimeout(() => this.lastScan.set(null), 1000);
  }
}
