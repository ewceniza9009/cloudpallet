...
import { InventoryApiService, RepackableInventoryDto, CreateKitCommand } from '../inventory-api.service';
import { map, startWith, switchMap, catchError, Observable, debounceTime, distinctUntilChanged, of } from 'rxjs';
...
export class KittingComponent implements OnInit {
  ...
  kitMaterials = signal<KitMaterialDto[]>([]);
  ...
  componentSourceFilters: Observable<RepackableInventoryDto[]>[] = []; 
  ...
  constructor() {
    ...
    effect(() => {
      ...
      this.filteredAccounts.set(
        accounts.filter((acc: AccountDto) => acc.name.toLowerCase().includes(filterValue))
      );
    });
    ...
    effect(() => {
      ...
      const filterValue = (typeof query === 'string' ? query : (query as KitMaterialDto)?.name || '').toLowerCase();
      this.filteredKitMaterials.set(
        kits.filter((mat: KitMaterialDto) => mat.name.toLowerCase().includes(filterValue) || mat.sku.toLowerCase().includes(filterValue))
      );
    });
  }
  ...
  async loadInitialLookups(): Promise<void> {
    ...
    try {
      ...
      this.allMaterials.set(allMaterials);

      // Optimization: Only fetch BOMs for materials that are likely to have them
      const kitMaterialPromises = allMaterials
        .filter((m: MaterialDetailDto) => m.materialType === 'Kit' || m.materialType === 'Repack')
        .map(async (material: MaterialDetailDto) => {
          try {
            const bom = await this.adminApi.getBomForMaterial(material.id).toPromise();
            return bom ? { ...material, bom } : null;
          } catch {
            return null;
          }
        });
      ...
      const kits = (await Promise.all(kitMaterialPromises))
        .filter((kit): kit is KitMaterialDto => kit !== null && (kit.bom.lines.length > 0));
  ...
  buildComponentForm(bom: BomDto): void {
    ...
    bom.lines.forEach((line: BomLineDto) => {
      const sourceSearchCtrl = new FormControl<string | RepackableInventoryDto>('');

      const componentGroup = this.fb.group({
        bomLine: [line],
        componentMaterialName: [this.allMaterials().find((m: MaterialDetailDto) => m.id === line.inputMaterialId)?.name || 'Unknown Material'],
        sourceInventoryId: [null, Validators.required],
        quantityToConsume: [bom.outputQuantity * line.inputQuantity, Validators.required],
      ...
      sourceSearchCtrl.valueChanges.subscribe((val: any) => {
     ...
     if (i === excludeIndex) continue;
      ...
      try {
        ...
        if (result && result.items.length > 0) {
          const match = result.items.find((item: any) => item.palletBarcode === palletBarcode);
          if (match) {
            group.patchValue({ 
